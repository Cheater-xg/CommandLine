...
Функция ПодготовитьФразуКОбработке(ТекСтрока) Экспорт 
	...
	Если ЭтотОбъект.ЕстьОбластьРаботыПользователя Тогда
	...
	Объект.ТаблицаПараметров.ЗаполнитьЗначения(Ложь, "Обработан");         
	//в таблице параметров могут оказаться дубли строк, избавимся от них
	Объект.ТаблицаПараметров.Свернуть("Параметр,Наименование,Тип,Обработан");
	МассивСтрокОчистить = Новый Массив;
	Для Каждого СтрПараметра Из Объект.ТаблицаПараметров Цикл
		...
		Иначе
			...
			СтрокиПоПараметру = Объект.ТаблицаПараметров.НайтиСтроки(СтруктураОтбора);
			СтрокаПараметров = "";
	...
	Для Каждого Элем Из МассивСтрокОчистить Цикл
		Объект.ТаблицаПараметров.Удалить(Элем);
	КонецЦикла;
				...
				Если Термин <> Неопределено Тогда
					...
					Если ТекИндекс = КоличествоСлов И ИндексТочноеСовпадение >= 0 Тогда
						...
						Пока ДопИндекс < КоличествоСловВСловаре 
							...
							НовПараметр = Объект.ТаблицаПараметров.Добавить();
...
Процедура ОбработатьРезультатНаКлиенте(Результат, ВидРезультата, ДопАтрибуты, НастройкиОбъекта, СтруктураПараметров) Экспорт
	...
	Если ВидРезультата = "отчет" Тогда
		...
		Если Объект.ТаблицаПараметров.Количество() > 0 Тогда
			Для Каждого ТекОтбор Из Объект.ТаблицаПараметров Цикл
	...
	ИначеЕсли ВидРезультата = "документсписок" ИЛИ ВидРезультата = "справочниксписок" Тогда
		...
		Для Каждого Стр ИЗ Объект.ТаблицаПараметров Цикл
...
// Начало, Каптюг А.А., ООО "Простые программы", 07.08.2017 
&НаСервере
Функция ПолучитьСтруктуруОписанияКонфигурации() Экспорт 

	Структура = Новый Структура;
	Структура.Вставить("Solution", 	Метаданные.Имя);
	Структура.Вставить("Developer", Метаданные.Поставщик);
	Структура.Вставить("Version", 	Лев(Метаданные.Версия, 3));
	Структура.Вставить("EasyRP_ID", " ");

	Возврат Структура;
	
КонецФункции // ПолучитьСтруктуруОписанияКонфигурации()
// Конец, Каптюг А.А., ООО "Простые программы", 07.08.2017
